...
the provided vNetClusterSubnetName. Defaults to `172.16.128.4`.

When deploying a new virtual network, the following parameters also apply

`vNetNewAddressPrefix`::
The address space when creating a new virtual network. Defaults to `172.16.128.0/24`.

`vNetNewClusterSubnetAddressPrefix`::
The address space of the `vNetClusterSubnetName` subnet. Defaults to `172.16.128.0/25`.
